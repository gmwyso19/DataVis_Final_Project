...
plot3 <- ggplot(data = Water_remove, aes(x = County, y = Concentration_Total)) +
 ...
 ggplotly(plot3, tooltip = c("County", "Concentration_Total")
...
Make a table to compare these rankings to try and make a point about is population size has a correlation with PFOA contamination concentration amounts.
```{r}
Water_parse <- Water_CA %>% rename(Population_Served = `Population Served`) %>% mutate(Population_Served = parse_number(Population_Served))

Water_parse %>% group_by(County)

Water_pop <- Water_parse %>% group_by(County) %>% summarise(Population_avg = mean(Population_Served)) %>% arrange(desc(Population_avg)) %>% mutate(Population_lower = Population_avg/1000) %>% ungroup()

Water_con <- Water_pop %>% group_by(County, Concentration, Population_lower) %>% summarise(Concentration_Total = mean(Concentration)) %>% arrange(desc(Concentration_Total)) 


view(Water_pop)

plot1 <- ggplot(data = Water_pop, aes(x = Concentration, y = Population_lower,
                                   color = County,      
                                   label = County)) +
  geom_point()
...
Water_coord2 <- full_join(Water_remove, county_coord, by = c("County" = "County"), copy = TRUE)
...
Water_coord
Water_coord2 <- Water_coord2 %>% mutate(Latitude = parse_number(Latitude)) 
Water_coord2 <- Water_coord2 %>% mutate(Longitude = parse_number(Longitude)) 


Water_coord2 <- Water_coord2 %>% slice(1:21)
#view(Water_coord2)
...
```{r}
library(urbnmapr)
Water_coord2 %>% ggplot(mapping = aes(-Longitude, Latitude, fill = Concentration_Total)) +
  geom_polygon(color = "#ffffff", size = .25) +
  coord_map(projection = "albers", lat0 = 39, lat1 = 45) +
  theme(legend.title = element_text(),
        legend.key.width = unit(.5, "in")) +
  labs(fill = "PFOA Average Concentration") +
  theme_urban_map()
```

Well that didn't work
```{r}
ggplot(data = Water_coord2, aes(x = Longitude, y = Latitude)) +
  geom_polygon(colour = "black", aes(fill = Concentration_Total)) +
  coord_map(projection = "albers", lat0 = 36.7783, lat1 = -119.4179) +
  theme_void() +
