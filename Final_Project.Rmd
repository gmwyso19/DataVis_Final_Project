...
Water_coord <- full_join(Water_remove, county_coord, by = c("County" = "County"), copy = TRUE)
...
```{r}
leaflet(data = Water_coord) %>% addProviderTiles('CartoDB') %>%
setView(lng = -119.4179, lat = 36.7783, zoom = 4.5) %>% addTiles() %>% 
  addProviderTiles(lng = ~Longitude,
                   lat = ~Latitude,
                   label = ~County) %>% 
  addMarkers(lng = Water_coord$Longitude, lat = Water_coord$Latitude,
