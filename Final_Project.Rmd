...
Water_CA.sp <- SpatialPointsDataFrame(Water_CA[])
m <- leaflet() %>% 
  addTiles()
m
```
...
Graph Showing Overall Concentrations Drawn in Each County During the Month of January in 2020.
```{r}
Water_CA$Concentration = as.numeric(Water_CA$Concentration)

Water_CA <- Water_CA %>% ungroup() %>% group_by(County) %>%
  summarise(Concentration_Total = sum(Concentration)) %>% arrange(desc(Concentration_Total)) 

Water_small <- Water_CA %>% slice(4:22)
Water_large <- Water_CA %>% slice(1:3)
#view(Water_CA)
ggplot(data = Water_small, aes(x = County, y = Concentration_Total)) +
  geom_point() + 
  geom_segment(aes(x = County, xend = County, y = 0, yend = Concentration_Total)) +
  coord_flip() +
  labs(x = "County Name", 
       y = "Concentrations of PFOA(NG/L) Drawn from each County in January 2020")

ggplot(data = Water_large, aes(x = County, y = Concentration_Total)) +
  geom_point() + 
  geom_segment(aes(x = County, xend = County, y = 0, yend = Concentration_Total)) +
  coord_flip() +
  labs(x = "County Name", 
       y = "Concentrations of PFOA(NG/L) Drawn from each County in January 2020")
```
Seperated lollipop graph to make it more readable. The counties with more instances of higher concentrations of PFOA are plotted second, while the counties with less instances of lower concentrations are plotted in the first lollipop graph. 

Scatterplot of Data (this needs adjusting)
```{r}
Water_rename <- Water_CA %>% rename(Population_Served = `Population Served`)
Water_CA$Concentration = as.numeric(Water_CA$Concentration)
...
Wondering if I should use the a less tidy version of the data for this graph, maybe show all of the readings from the year 2020 rather than condensing it down to a graph.
...
" I keep getting this error in my mappings!
