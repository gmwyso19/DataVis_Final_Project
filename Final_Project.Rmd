...
plot3 <- ggplot(data = Water_remove, aes(x = County, y = Concentration_Total)) +
 ...
 ggplotly(plot3, tooltip = "Concentration_Total")
...
Water_pop <- Water_parse %>% group_by(County, Concentration) %>% summarise(Population_lower = mean(Population_Served)) %>% arrange(desc(Population_lower)) %>% mutate(Population_lower = Population_Served/1000) %>% ungroup()

Water_con <- Water_parse %>% group_by(County) %>% summarise(Concentration_Total = mean(Concentration)) %>% arrange(desc(Concentration_Total))
