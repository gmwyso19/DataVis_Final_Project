...
Water_AVG <- Water_CA%>% ungroup() %>% group_by(County) %>%
  summarise(Concentration_Total = mean(Concentration)) %>% arrange(desc(Concentration_Total))
...
```
Just need to figure out how to arrange them in descending order)

```{r}

ggplot(data = Water_rename, aes(x = County, y = Population_Served)) +
  geom_point(size=3, color="blue", fill=alpha("light blue", 0.3), alpha=0.7, shape=21, stroke=1) + 
  geom_segment(aes(x = County, xend = County, y = 0, yend = Population_Served)) +
  coord_flip() +
  labs(x = "County Name", 
...
Lost 329 rows of population data after making the data numeric, why?
...
```{r}
Water_rename$Population_Served = as.numeric(Water_rename$Population_Served)
...
Trying to make population lower and inorder