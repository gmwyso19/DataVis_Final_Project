...
Water_CA <- Water_CA %>% ungroup() %>% group_by(County) %>%
  summarise(Concentration_Total = sum(Concentration)) %>% slice(2:22) %>% arrange(desc(Concentration_Total))

ggplot(data = Water_CA, aes(x = County, y = Concentration_Total)) +
  geom_point() + 
  geom_segment(aes(x = County, xend = County, y = 0, yend = Concentration_Total)) +
  coord_flip() +
  labs(x = "County Name", 
       y = "Number of Concentrations Drawn from each County in January 2020")
```
